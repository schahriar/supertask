...
var BUCKETSORT = function METHOD_BUCKETSORT(array, func) {    
    ...
    return BUCKETSORT(left, func).concat(next).concat(BUCKETSORT(right, func));
};
...
module.exports = function HYBRID_SORT(array, func, algorithm) {
    ...
    */
    ...
    algorithm = algorithm || ((array.length > 800)?'QS':'BS');
    // Run Sort
    if (algorithm === 'QS') return QUICKSORT(array, func);
    if (algorithm === 'BS') return BUCKETSORT(array, func);
