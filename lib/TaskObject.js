...
class Task {
    constructor(instance, name, func, handler, type, model) {
        ...
        // Attach call methods
        this.model.context = this._define_call_methods(name);
    }
    
    _define_call_methods(name) {
        var apply = this._apply.bind(this);
        var call = this._do.bind(this);
        var self = this.model;
        return {
            apply: function SUPERTASK_DEEP_APPLIER(name, context, args, callback) {
                apply(name, context, args, callback);
            },
            call: function SUPERTASK_DEEP_CALLER() {
                call.apply(null, arguments);
            },
            recurse: function SUPERTASK_DEEP_RECURSER(args, callback) {
                apply(name, this, args, callback);
            },
            self: {
                executionRounds: self.executionRounds,
                averageExecutionTime: self.averageExecutionTime,
                name: self.name,
                lastStarted: self.lastStarted,
                lastFinished: self.lastFinished,
    ...
    */
    precompile(context) {
        if (this.remote) throw new Error('It\'s currently not possible to pre-compile a remote task');
        // Compile script
