...
var Optimizer = function ST_OPTIMIZER(array, O_LEVEL, O_MASK) {
    ...
    array.forEach(function ST_OPTIMIZER_ARRAY_MIN_MAX_SETTER(o) {
        ...
        for (i = 0; i < ST_O_PROPERTIES.length; i++) {
            if ((!M1[ST_O_PROPERTIES[i].n]) || (M1[ST_O_PROPERTIES[i].n] < o[ST_O_PROPERTIES[i].n])) {
                M1[ST_O_PROPERTIES[i].n] = o[ST_O_PROPERTIES[i].n] * 1;
            } else if ((!M2[ST_O_PROPERTIES[i].n]) || (M2[ST_O_PROPERTIES[i].n] > o[ST_O_PROPERTIES[i].n])) {
                M2[ST_O_PROPERTIES[i].n] = o[ST_O_PROPERTIES[i].n] * 1;
            }
        }
    });
    ...
    var OPTIMIZER_MAPPED_ARRAY = array.map(function ST_OPTIMIZER_ARRAY_MAPPER(o, index) {
        ...
        for (i = 0; i < ST_O_PROPERTIES.length; i++) {
            ...
            if((ST_O_PROPERTIES[i].rule) && (O_MASK & ST_O_PROPERTIES[i].rule.dsc)) {
                ...
                (
                    (
                        (o[ST_O_PROPERTIES[i].n] * 1 - M1[ST_O_PROPERTIES[i].n]) /
                        (M2[ST_O_PROPERTIES[i].n] - M1[ST_O_PROPERTIES[i].n])
                    ) * ST_O_PROPERTIES[i].i
                ) || 0;
            }else{
                REDUCED_VALUE += 
                (
                    (
                        (o[ST_O_PROPERTIES[i].n] * 1 - M2[ST_O_PROPERTIES[i].n]) /
                        (M1[ST_O_PROPERTIES[i].n] - M2[ST_O_PROPERTIES[i].n])
                    ) * ST_O_PROPERTIES[i].i
                ) || 0;
            }
        }
