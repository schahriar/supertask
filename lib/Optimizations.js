...
/* - Property n represents name
   ...
   - Property rule is an object with ascending (asc) & descending (dsc) flags
   - Property MIN_O_LEVEL determines the minimum optimization level the PROP is used at
*/
var PROP = [
    { n: "averageExecutionTime", i: 0.4, rule: { asc: ST_O_AET_ASC, dsc: ST_O_AET_DSC }, MIN_O_LEVEL: ST_O2 },
    { n: "executionRounds", i: 0.15, rule: { asc: ST_O_ER_ASC, dsc: ST_O_ER_DSC }, MIN_O_LEVEL: ST_O2 },
    { n: "priority", i: 0.45, rule: { asc: ST_O_PRIORITY_ASC, dsc: ST_O_PRIORITY_DSC }, MIN_O_LEVEL: ST_O1 }
];
...
var Optimizer = function ST_OPTIMIZER(array, O_LEVEL, O_MASK) {
    ...
    array.forEach(function ST_OPTIMIZER_ARRAY_MIN_MAX_SETTER(o) {
        ...
        for (i = 0; i < PROP.length; i++) {
            // Skip if Optimization level doesn't match
            if (O_LEVEL < PROP[i].MIN_O_LEVEL) continue;
    ...
    var OPTIMIZER_MAPPED_ARRAY = array.map(function ST_OPTIMIZER_ARRAY_MAPPER(o, index) {
        ...
        for (i = 0; i < PROP.length; i++) {
            ...
            // Make sure value is absolute (positive)
            if (o[PROP[i].n] < 0) {
                /* Revisit & write more tests */
                // Set iteration value to minimum in order to push back
                REDUCED_VALUE += M2[PROP[i].n];
                continue;
            }
            ...
            /*
             ...
             -- Formula -- (M1/M2 are interchangeable based on order)
                (value - M2)/(M1 - M2) * PROP[i].i * ((Order Decision)?normal:reversed)
                ||
                0
            */
            REDUCED_VALUE +=
            (
                (
                    ...
                    ) * PROP[i].i * (((PROP[i].rule) && (O_MASK & PROP[i].rule.dsc)) ? 1 : -1)
                ) || 0;
        }
